...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    if((pte = walk(pagetable, a, 0)) == 0)
      continue;   // Skip - lazy page (no PTE)
    if((*pte & PTE_V) == 0)
      continue;   // Skip - lazy page (invalid PTE)
    if(do_free){
...
// ========== MODIFIED BY MEMBER 4 ==========
// Skip lazy pages during fork (no physical memory to copy)
int
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    if((pte = walk(old, i, 0)) == 0)
      continue;   // Skip - lazy page in parent
    if((*pte & PTE_V) == 0)
      continue;   // Skip - lazy page in parent
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
...
lazy_alloc(pagetable_t pagetable, uint64 va)
{
  ...
  // Map virtual to physical with user permissions
